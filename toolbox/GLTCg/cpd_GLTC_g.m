%   [C, W, sigma2, iter, T] =cpd_GLTC_g(X, Y, neighbors, beta, lambda, max_it, tol, viz, outliers, fgt, corresp,sigma2);
%
%   Input
%   ------------------
...
%	neighbors	the index of neighbor nodes on geodesic paths
%   beta       (try 1..5 ) std of Gaussian filter 
...
function  [C, W, sigma2, iter, T] =cpd_GLTC_g(X, Y, neighbors, beta, lambda, max_it, tol, viz, outliers, fgt, corresp,sigma2);
...
%while (iter<max_it)  && (sigma2 > 1e-8) %(sigma2 > 1e-8)
    ...
    ntol=abs((L-L_old)/L);
%     disp([' GLTC_g ' st ' : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]);
