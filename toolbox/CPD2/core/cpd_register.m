...
function [Transform, C]=cpd_register(X, Y, opt, Neighbors)
...
switch lower(opt.method),
    ...
    case 'gltc' %Zhiwei 
        [C, W, sigma2, iter, T] =cpd_GLTC(X, Y, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.fgt, opt.corresp, opt.sigma2);    
    case 'gltcg' %Zhiwei 
        [C, W, sigma2, iter, T] =cpd_GLTC_g(X, Y, Neighbors, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.fgt, opt.corresp, opt.sigma2);    
    otherwise
...
%     case {'nonrigid','nonrigid_lowrank'}
    case {'nonrigid','nonrigid_lowrank','gltc', 'gltcg'}
